...
for file in study/*
    do  
        python ./goatools/scripts/find_enrichment.py --ev_inc=${10:-Experimental} --pval=1 --ns=${9:-BP} --pval_field=fdr_bh --outfile=$PWD/xls/$(echo $file | cut -d/ -f2).xlsx --no_propagate_counts --method=fdr_bh --obo=go-basic.obo ${file} population_tmp_$6 tmp_$7
    done

#Part for EGAD on each cluster
for file2 in cluster_links/*
    do
      python EGAD_preprocessing.py ${file2} $7 --process ${9:-BP} --ev_inc ${10:-Experimental} --specific
      Rscript --vanilla GO_enrichment.R tmp_adj_matrix.csv tmp_GO_matrix.csv AUROC_${file2} PR_${file2}
      if [ $5 = 'HRR' ]
        then
          python GO_comparison.py AUROC_${file2} PR_${file2} $1 $2 $3 $4 tmp_adj_matrix.csv --$5
        else
          python GO_comparison.py AUROC_${file2} PR_${file2} $1 $2 $3 $4 tmp_adj_matrix.csv
      fi
    done
