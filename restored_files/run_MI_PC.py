...
from cdt.independence.stats.numerical import SpearmanCorrelation
...
# Exec computations
print("Number of cores to use for parallelization : {}".format(args.njobs))
...
if(args.header):
    ...
    print("Done...")
    print("Writing dataframe into csv file...")
    report.to_csv(args.output,index=True,header=True)
    print("Done...")
else:
    report = pd.DataFrame(results)
    report.to_csv(args.output,index=False,header=False)
